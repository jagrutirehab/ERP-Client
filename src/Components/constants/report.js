export const DASHBOARD = "DASHBOARD";
export const REPORT = "REPORT";
export const FINANACE = "FINANACE";
export const DB_LOGS = "DB_LOGS";
export const PATIENT_ANALYTICS = "PATIENT_ANALYTICS";
export const DOCTOR_ANALYTICS = "DOCTOR_ANALYTICS";
export const LEAD_ANALYTICS = "LEAD_ANALYTICS";
export const OPD_ANALYTICS = "OPD_ANALYTICS";
export const HUBSPOT_CONTACTS = "HUBSPOT_CONTACTS";
export const BOOKING = "BOOKING";
export const CENTER_BEDS_ANALYTICS = "CENTER_BEDS_ANALYTICS";
export const MI_REPORTING = "MI_REPORTING";
export const ADMISSION_FORMS = "ADMISSION_FORMS";
export const REPORT_UPDATED = "REPORT_UPDATED";
export const ADVANCE_PAYMENT_DEPOSIT = "ADVANCE_PAYMENT_DEPOSIT";
