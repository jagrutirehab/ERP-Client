...
const styles = StyleSheet.create({
  ...
  fontMd: { fontSize: "10px" },
  textCaps: { textTransform: "capitalize" },
});

const border = "1px solid #000";

const Header = ({ date, patient, user }) => {
  const age = () =>
    patient?.dateOfBirth
      ...
      <View>
        ...
        >
          <Text style={styles.textCaps}>
            Printed By: {capitalizeWords(user?.name) || ""}
          </Text>
          <Text>
            On: {date ? format(new Date(date), "dd MMM yyyy hh:mm a") : ""}
          </Text>
        </View>
