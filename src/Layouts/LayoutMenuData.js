...
const Navdata = () => {
  ...
  const sortPages = (routes) => {
    ...
    routes?.sort((a, b) => {
      ...
      // If both items are in the sortOrder array, compare their positions
      if (indexA !== -1 && indexB !== -1) {
        return indexA - indexB;
      }

      // If only one of the items is in sortOrder, prioritize the one that is
      if (indexA !== -1) {
        return -1;
      }
      ...
      // If neither is in sortOrder, maintain the original order
      return 0;
    });
