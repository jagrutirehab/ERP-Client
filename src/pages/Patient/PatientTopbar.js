...
const PatientTopbar = ({
  ...
  const handleUnAssignNurse = async () => {
    try {
      await dispatch(unAssignNurse(patient?._id)).unwrap();
             ...
             {/* <CheckPermission allowedRoles={["ADMIN","MANAGER"]} userRole={user?.role}>
                ...
                >
                  {assignedNurse?.name ? (
                    ...
                    >
                      <RenderWhen isTrue={nurseLoading}>
                        <Spinner size="sm" className="me-2" />
                      </RenderWhen>
                      <span>Unassign Nurse</span>
                    </Button>
                  ) : (
                    <Button
                      color="primary"
