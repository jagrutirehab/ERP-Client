...
import {
  createCOWSTest, // ⚠️ Add this action to clinicalTestSlice
  setIsClinicalTab,
  setTestName,
  setTestPageOpen,
...
} from "./QuestionData/COWSQuestions";
import { useAuthError } from "../../../../Components/Hooks/useAuthError";

const COWSAssessment = () => {
  ...
  const closeModal = () => {
    const unanswered = cowsQuestions.filter(
  ...
  const handleSubmit = async (e) => {
    ...
    const formattedQuestions = cowsQuestions
    ...
    try {
      await dispatch(createCOWSTest(formData)).unwrap();
      openModal(
      ...
      <div className="mb-5 p-4 bg-light border border-primary rounded shadow-sm">
        ...
        <ul className="ps-3 text-secondary small fs-6">
          <li className="mb-2 text-black">
            <strong>Purpose:</strong> The COWS provides a standardised
            clinician-rated assessment of opiate withdrawal severity across 11
            observable signs and symptoms.
          </li>
          <li className="mb-2 text-black">
            <strong>Administration:</strong> Observe the patient directly for
            signs such as sweating, restlessness, tremor, yawning, and
            piloerection. Supplement with patient self-report where indicated.
          </li>
          <li className="mb-2 text-black">
            <strong>Scoring:</strong> Each item is scored independently. Sum all
            item scores for the total. Do not leave items blank.
          </li>
          <li className="mb-2 text-black">
            <strong>Interpretation:</strong> 5–12 = Mild; 13–24 = Moderate;
            25–36 = Moderately Severe; &gt;36 = Severe withdrawal.
          </li>
          <li className="mb-2 text-black">
            <strong>Reassessment:</strong> Reassess every 1–4 hours depending on
