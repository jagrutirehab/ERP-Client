...
import { createCiwaTest, fetchCiwaTest, fetchDoctors, fetchPatientById } from '../../../../store/actions';
import { Dropdown, DropdownItem, DropdownMenu, DropdownToggle } from 'reactstrap';
import Loader from '../../../../Components/Common/Loader';
import { setIsClinicalTab, setTestName, setTestPageOpen } from '../../../../store/features/clinicalTest/clinicalTestSlice';
...
const CiwaQuestions = () => {
    ...
    const ciwaQuestions = [
        {
            id: 'nauseaVomiting',
            question: '1. Nausea and Vomiting: Do you feel sick to your stomach? Have you vomited?',
            type: 'radio',
            options: [
                ...
                { score: 7, label: '7: Constant nausea, frequent dry heaves & vomiting' },
            ],
            guideline: 'Observe patient. Ask specific questions about nausea and vomiting. Rate based on severity and frequency.',
        },
        {
            id: 'tremor',
            question: '2. Tremor: (Observe patient with arms extended, fingers spread apart) How severe are your tremors?',
            type: 'radio',
            options: [
                { score: 0, label: '0: No tremor' },
                { score: 1, label: '1: Not visible, but can be felt fingertip to fingertip' },
            ...
            guideline: 'Observe patient\'s hands with arms extended. Rate based on visibility and intensity of tremor.',
        },
        {
            id: 'paroxysmalSweats',
            question: '3. Paroxysmal Sweats: (Observe patient) How much are you sweating?',
            type: 'radio',
            options: [
            ...
            guideline: 'Observe for visible sweating, especially on the forehead and palms. Rate based on amount of sweat.',
        },
        {
            id: 'anxiety',
            ...
            options: [
                ...
                { score: 4, label: '4: Moderately anxious, or guarded, so anxiety is inferred' },
                { score: 5, label: '5: Acute panic states' },
                { score: 6, label: '6: Acute panic states' },
                { score: 7, label: '7: Equivalent to acute panic states as seen in severe delirium or acute schizophrenic reactions' },
            ],
            guideline: 'Ask the patient directly. Observe for non-verbal cues of anxiety. Rate based on expressed feelings and observed behavior.',
        },
        {
            id: 'agitation',
            ...
            options: [
                ...
                { score: 7, label: '7: Paces back and forth during most of the interview, or constantly thrashes about' },
            ],
            guideline: 'Observe patient\'s motor activity and restlessness throughout the interview. Rate based on the degree of physical activity.',
        },
        {
            id: 'tactileDisturbances',
            question: '6. Tactile Disturbances: Have you any itching, pins and needles sensations, any burning, any numbness, or do you feel bugs crawling on or under your skin?',
            type: 'radio',
            options: [
                { score: 0, label: '0: None' },
                { score: 1, label: '1: Very mild itching, pins & needles, burning or numbness' },
                { score: 2, label: '2: Mild itching, pins & needles, burning or numbness' },
                { score: 3, label: '3: Moderate itching, pins & needles, burning or numbness' },
            ...
            guideline: 'Ask specific questions about tactile sensations. Distinguish between general discomfort and explicit hallucinatory experiences.',
        },
        {
            id: 'auditoryDisturbances',
            question: '7. Auditory Disturbances: Are you more aware of sounds around you? Are they harsh? Do they frighten you? Are you hearing anything that is disturbing to you? Are you hearing things you know are not there?',
            type: 'radio',
            options: [
            ...
            guideline: 'Ask about auditory experiences. Differentiate between heightened sensitivity to sounds and actual auditory hallucinations.',
        },
        {
            id: 'visualDisturbances',
            question: '8. Visual Disturbances: Does the light appear to be too bright? Is its color different? Does it hurt your eyes? Are you seeing anything that is disturbing to you? Are you seeing things you know are not there?',
            type: 'radio',
            options: [
                { score: 0, label: '0: Not present' },
            ...
            guideline: 'Inquire about visual sensitivity and any visual distortions or hallucinations. Pay attention to patient descriptions of visual experiences.',
        },
        {
            id: 'headache',
            question: '9. Headache, Fullness in Head: Does your head feel different? Does it feel like there is a band around your head? (Do not rate for dizziness or lightheadedness)',
            type: 'radio',
            options: [
                { score: 0, label: '0: Not present' },
            ...
            guideline: 'Ask about headache and sensation of fullness. Ensure not to rate for dizziness or lightheadedness, only pain or pressure in the head.',
        },
        {
            id: 'orientation',
            question: '10. Orientation and Clouding of Sensorium: What day is this? Where are you? Who am I?',
            type: 'radio',
            options: [
                { score: 0, label: '0: Oriented and can do serial additions (e.g., 3, 6, 9...)' },
                { score: 1, label: '1: Cannot do serial additions or is uncertain about date' },
                { score: 2, label: '2: Disoriented for date by no more than 2 calendar days' },
                { score: 3, label: '3: Disoriented for date by more than 2 calendar days' },
                { score: 4, label: '4: Disoriented for place and/or person' },
            ],
            guideline: 'Assess patient\'s orientation to person, place, and time. Also, test their ability to perform serial additions (e.g., subtracting 7 from 100, or adding 3s).',
        },
    ];
    ...
    const handleSubmit = async (event) => {
        ...
        if (selectedDoctor.id === null || selectedDoctor.id == -1 || selectedDoctor.id == "-1") {
        ...
        openModal('Test submitted! The results are now available on the next page.');
    ...
    return (
        <div className="d-flex  align-items-center justify-content-center min-vh-100 px-3 px-sm-4 px-lg-5 text-dark px-3  " style={{ background: "linear-gradient(to bottom right, #e0e7ff, #ddd6fe); font-family: 'Inter', sans-serif" }} >
            ...
            <div className="bg-white shadow-xl rounded-2xl p-6 sm:p-8 lg:p-10 w-full max-w-4xl ">
                ...
                {userId && (
                    <div className="mb-4 d-flex align-items-center justify-content-between  p-3 border border-primary rounded text-primary small bg-light">
                        <div className="d-flex " style={{ flexDirection: "column", justifyContent: "start" }}>
                        ...
                        <div className='d-flex ' style={{ gap: "5px" }} >
                            ...
                            >
                                ...
                                <DropdownMenu flip={false}>
                                    ...
                                    {doctorDetails && doctorDetails.length>0 && doctorDetails.map((item, idx) => (
                                        <DropdownItem
                                            key={item._id}
                                            onClick={() => {
                    ...
                    <>
                        ...
                        <div className="mb-5 p-4 bg-light border border-primary rounded shadow-sm">
                            ...
                            <ul className="ps-3 text-secondary text-dark small fs-6">
                                <li className="mb-2  ">Ensure a calm and private environment for the assessment.</li>
                                <li className="mb-2">Read each question clearly and exactly as written.</li>
                                <li className="mb-2">Listen carefully to the patient's responses and observe their behavior.</li>
                                <li className="mb-2">Rate each item objectively based on the provided scoring criteria.</li>
                                <li className="mb-2">For subjective items, infer anxiety/agitation from patient's affect and behavior if not explicitly stated.</li>
                                <li className="mb-2">This tool helps assess alcohol withdrawal severity; use it in conjunction with clinical judgment.</li>
                                <li className="mb-2">Document any additional observations or relevant patient information.</li>
                            </ul>
                        </div>
                        ...
                        <form onSubmit={handleSubmit} className="mb-5">
                            {ciwaQuestions.map((q) => (
                                <div key={q.id} className="mb-4 p-4 bg-white border rounded shadow-sm">
                                    <h3 className="h6 h5-sm fw-semibold text-dark mb-2 fx-4 ">{q.question}</h3>
