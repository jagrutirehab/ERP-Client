...
import {
  createCGISTest, // ⚠️ Add this action to clinicalTestSlice
  setIsClinicalTab,
  setTestName,
  setTestPageOpen,
...
} from "./QuestionData/CGISQuestions";
import { useAuthError } from "../../../../Components/Hooks/useAuthError";

const CGISAssessment = () => {
  ...
  const closeModal = () => {
    const unanswered = cgisQuestions.filter(
  ...
  const handleSubmit = async (e) => {
    ...
    const formattedQuestions = cgisQuestions
    ...
    try {
      await dispatch(createCGISTest(formData)).unwrap();
      openModal(
      ...
      <div className="mb-5 p-4 bg-light border border-primary rounded shadow-sm">
        ...
        <ul className="ps-3 text-secondary small fs-6">
          <li className="mb-2 text-black">
            <strong>Purpose:</strong> The CGI-S provides a standardised
            clinician-rated assessment of the overall severity of a patient's
            mental illness at the time of assessment.
          </li>
          <li className="mb-2 text-black">
            <strong>Administration:</strong> Rate the patient based on your
            total clinical experience with this population. Consider symptoms,
            behaviour, and functional impairment.
          </li>
          <li className="mb-2 text-black">
            <strong>Single Selection:</strong> Select only one rating that best
            reflects the patient's current severity compared to patients with
            the same diagnosis.
          </li>
          <li className="mb-2 text-black">
            <strong>Reference:</strong> Scores 1–2 indicate normal to
            borderline; 3–4 mild to moderate illness; 5–7 marked to extremely
            severe illness.
          </li>
          <li className="mb-2 text-black">
            <strong>Reassessment:</strong> Reassess at each clinical encounter
