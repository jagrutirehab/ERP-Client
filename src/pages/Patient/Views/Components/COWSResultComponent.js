const COWSResultComponent = ({ resultData }) => {
  const {
    systemTotalScore,
  ...
  const getSeverityColor = (severity) => {
    if (severity === "Below Mild (No significant withdrawal)")
      return "text-success";
    if (severity === "Mild Withdrawal") return "text-warning";
    if (severity === "Moderate Withdrawal") return "text-warning";
    if (severity === "Moderately Severe Withdrawal") return "text-danger";
    if (severity === "Severe Withdrawal") return "text-danger";
    return "text-primary";
  };
    ...
    <div className="">
      <div className="p-4 bg-light border border-primary rounded shadow-lg">
        <h2 className="h5 h4-sm fw-semibold text-primary mb-3">
          ...
          Clinical Opiate Withdrawal Scale (COWS) Results
        </h2>
...
export default COWSResultComponent;
