...
import {
  ...
  ramsayQuestions,
} from "./QuestionData/Ramsayquestions";
import { useAuthError } from "../../../../Components/Hooks/useAuthError";

const RamsayAssessment = () => {
  ...
  const closeModal = () => {
    const unanswered = ramsayQuestions.filter(
  ...
  const handleSubmit = async (e) => {
    ...
    const formattedQuestions = ramsayQuestions
    ...
    try {
      await dispatch(createRamsayTest(formData)).unwrap();
      openModal(
      ...
      <div className="mb-5 p-4 bg-light border border-primary rounded shadow-sm">
        ...
        <ul className="ps-3 text-secondary small fs-6">
          <li className="mb-2 text-black">
            <strong>Purpose:</strong> The Ramsay Sedation Scale provides a
            standardised assessment of the level of sedation in critically ill
            or sedated patients.
          </li>
          <li className="mb-2 text-black">
            <strong>Administration:</strong> Observe the patient's current
            state. For patients who appear asleep (scores 4–6), apply a light
            glabellar tap or a loud auditory stimulus and observe the response.
          </li>
          <li className="mb-2 text-black">
            <strong>Single Selection:</strong> Select only one level that best
            describes the patient's state at the time of assessment. This is not
            a summation scale.
          </li>
          <li className="mb-2 text-black">
            <strong>Target Range:</strong> Scores 2–3 are generally considered
            optimal for most ICU patients not requiring deep sedation.
          </li>
          <li className="mb-2 text-black">
            <strong>Reassessment:</strong> Reassess regularly, especially
            following any change in sedative agents or infusion rates.
          </li>
        </ul>
      </div>

      {ramsayQuestions.map((q, idx) => (
...
export default RamsayAssessment;