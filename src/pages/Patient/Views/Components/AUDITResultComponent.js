const AUDITResultComponent = ({ resultData }) => {
  const {
    systemTotalScore,
  ...
  const getSeverityColor = (severity) => {
    if (severity === "Low Risk") return "text-success";
    if (severity === "Hazardous Use") return "text-warning";
    if (severity === "Harmful Use") return "text-danger";
    if (severity === "Possible Dependence") return "text-danger";
    return "text-primary";
  };
    ...
    <div className="">
      <div className="p-4 bg-light border border-primary rounded shadow-lg">
        <h2 className="h5 h4-sm fw-semibold text-primary mb-3">
          ...
          Alcohol Use Disorders Identification Test (AUDIT) Results
        </h2>
...
export default AUDITResultComponent;
