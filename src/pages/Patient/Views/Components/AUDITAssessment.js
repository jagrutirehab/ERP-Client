...
import {
  ...
  getVisibleQuestions,
} from "./QuestionData/AUDITQuestions";
...
const AUDITAssessment = () => {
  ...
  // Questions to render — updated reactively as answers change
  ...
  const handleSubmit = async (e) => {
    ...
    // Only validate visible (non-skipped) questions
    const unanswered = visibleQuestions.filter((q) => !answers[q.id]);
    if (unanswered.length > 0) {
      return openModal("Please answer all the questions");
    }

    // Apply skip logic — fills skipped questions with their 0-score option
    const filledAnswers = applySkipLogic(answers);
    const totalScore = calculateScores(filledAnswers);
    const { severity, interpretation, recommendations } =
      getInterpretationAndRecommendations(totalScore);

    const formattedQuestions = auditQuestions.map((q) => ({
      questionId: q.id,
      question: q.question[language],
      answer: filledAnswers[q.id] || "",
      score: q.score?.[filledAnswers[q.id]] ?? 0,
    }));
    ...
    try {
      await dispatch(createAUDITTest(formData)).unwrap();
      openModal(
  ...
  return (
    <div className="p-2 p-sm-3">
      {/* Patient Info + Doctor/Language Dropdowns */}
      ...
      <div className="mb-5 p-4 bg-light border border-primary rounded shadow-sm">
        ...
        <ul className="ps-3 text-secondary small fs-6">
          <li className="mb-2 text-black">
            <strong>Purpose:</strong> The AUDIT is a 10-item WHO screening tool
            to identify hazardous, harmful, and dependent alcohol use.
          </li>
          <li className="mb-2 text-black">
            <strong>Recall Period:</strong> All questions refer to the past year
            unless otherwise noted.
          </li>
          <li className="mb-2 text-black">
            <strong>Skip Logic:</strong> If Q1 = Never, questions 2–8 are
            skipped automatically. If Q2 and Q3 both score 0, questions 4–8 are
            skipped automatically. Skipped items score 0.
          </li>
          <li className="mb-2 text-black">
            <strong>Scoring:</strong> Q1–Q8 score 0–4; Q9 and Q10 score 0, 2, or
            4. Total range is 0–40.
          </li>
          <li className="mb-2 text-black">
            <strong>Interpretation:</strong> 0–7 = Low risk; 8–15 = Hazardous;
            16–19 = Harmful; ≥20 = Possible dependence.
          </li>
        </ul>
      </div>

      {/* Questions — only visible questions rendered */}
