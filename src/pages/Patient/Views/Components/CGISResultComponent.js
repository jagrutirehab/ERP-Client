const CGISResultComponent = ({ resultData }) => {
  const {
    systemTotalScore,
  ...
  const getSeverityColor = (severity) => {
    if (severity === "Normal, not at all ill") return "text-success";
    if (severity === "Borderline mentally ill") return "text-success";
    if (severity === "Mildly ill") return "text-warning";
    if (severity === "Moderately ill") return "text-warning";
    if (severity === "Markedly ill") return "text-danger";
    if (severity === "Severely ill") return "text-danger";
    if (severity === "Among the most extremely ill patients")
      return "text-danger";
    return "text-primary";
  };
    ...
    <div className="">
      <div className="p-4 bg-light border border-primary rounded shadow-lg">
        <h2 className="h5 h4-sm fw-semibold text-primary mb-3">
          ...
          Clinical Global Impression — Severity (CGI-S) Results
        </h2>
...
export default CGISResultComponent;
