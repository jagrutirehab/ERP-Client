// After iPhone long recordings issue 
...
const AudioRecorder = ({ onReady }) => {
  ...
  const mergeIntervalRef = useRef(null);
  useEffect(() => {
  ...
  const handleStart = () => {
    if (mediaRecorderRef.current) {
      ...
      mediaRecorderRef.current.start(isIOS ? 5000 : undefined);

      if (isIOS) {
        mergeIntervalRef.current = setInterval(() => {
          if (audioChunksRef.current.length > 1) {
            const merged = new Blob(audioChunksRef.current, { type: mimeTypeRef.current || "audio/mp4" });
...
// Sarang's Updated on prod 11/04/2026
// import React, { useEffect, useState, useRef } from "react";
// import { Button, Alert } from "reactstrap";
...
//       mediaRecorderRef.current.start();
//       // const isIOS = /iPhone|iPad|iPod/i.test(navigator.userAgent);
//       // mediaRecorderRef.current.start(isIOS ? 10000 : undefined);
//       setIsRecording(true);
//     }
...
// Old Code (Initial) - Before 11/04/2026
