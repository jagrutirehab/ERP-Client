...
const Questionary = ({ questionary, onChange, isSubmitted }) => {
    ...
    const getQuestionError = (q) => {
        ...
        if (q.options.some(o => !o.text.trim())) return 'All option texts are required'
        if (!q.options.some(o => o.isCorrect)) return 'Mark at least one correct answer'
        return null
    }
        ...
        <div style={styles.wrapper}>
            <div style={styles.sectionHeader}>
                <div>
                    ...
                    <p style={styles.sectionSub}>Optional — add MCQ questions for this training</p>
                </div>
            ...
            {questionary.length === 0 && (
                <div style={styles.emptyState}>
                    <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" style={{ color: 'var(--color-text-tertiary)', marginBottom: 8 }}>
                        <circle cx="12" cy="12" r="10" />
                        <path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3" />
                        <line x1="12" y1="17" x2="12.01" y2="17" />
                    </svg>
                    <p style={styles.emptyText}>No questions yet. Click "Add Question" to begin.</p>
                </div>
            )}
            ...
            {questionary.map((q, qIdx) => {
                ...
                return (
                    <div key={q.id} style={{ ...styles.questionCard, ...(error ? styles.questionCardError : {}) }}>
                        ...
                        <input
                            type="text"
                            style={{ ...styles.questionInput, ...(isSubmitted && !q.question.trim() ? styles.inputError : {}) }}
                        ...
                        <div style={styles.multipleToggleRow}>
                            <label style={styles.toggleLabel}>
                                <div
                                    style={{ ...styles.toggle, ...(q.allowMultiple ? styles.toggleOn : styles.toggleOff) }}
                                    onClick={() => updateQuestion(q.id, 'allowMultiple', !q.allowMultiple)}
                                    role="switch"
                                    aria-checked={q.allowMultiple}
                                    tabIndex={0}
                                    onKeyDown={e => e.key === ' ' && updateQuestion(q.id, 'allowMultiple', !q.allowMultiple)}
                                >
                                    <div style={{ ...styles.toggleThumb, ...(q.allowMultiple ? styles.toggleThumbOn : {}) }} />
                                </div>
                                <span style={styles.toggleText}>
                                    Multiple correct answers
                                    <span style={styles.toggleHint}>
                                        {q.allowMultiple ? '(Checkbox — select all that apply)' : '(Radio — one answer only)'}
                                    </span>
                                </span>
                            </label>
                        </div>
                        ...
                        <div style={styles.optionsList}>
                            {q.options.map((opt, oIdx) => (
                                <div key={opt.id} style={styles.optionRow}>
                                    <div style={styles.optionIndex}>{String.fromCharCode(65 + oIdx)}</div>

                                    <input
                                        type="text"
                                        style={{ ...styles.optionInput, ...(isSubmitted && !opt.text.trim() ? styles.inputError : {}) }}
                                        placeholder={`Option ${String.fromCharCode(65 + oIdx)}`}
                                        value={opt.text}
                                        onChange={e => updateOption(q.id, opt.id, 'text', e.target.value)}
                                    />

                                    <label style={styles.correctLabel} title={q.allowMultiple ? 'Mark as correct' : 'Mark as correct answer'}>
                                        <input
                                        ...
                                        />
                                        <div style={{ ...styles.correctIndicator, ...(opt.isCorrect ? styles.correctIndicatorActive : {}) }}>
                                            {opt.isCorrect && (
                                        ...
                                        <span style={{ ...styles.correctText, ...(opt.isCorrect ? styles.correctTextActive : {}) }}>
                                            Correct
                                        </span>
                                    </label>
