...
const ManageRulesHeader = ({ counts, loading, onNew, onRefresh }) => (
  <div className="d-flex align-items-start justify-content-between mb-3 flex-wrap gap-2">
    <div>
      <h1 className="display-6 fw-bold text-primary mb-1">MANAGE RULES</h1>
      <p className="text-muted mb-0">
        <strong>{counts.total}</strong> rule{counts.total === 1 ? "" : "s"} total ·{" "}
    ...
    <div className="d-flex gap-2">
      ...
      <Button color="secondary" outline size="sm" onClick={onRefresh} disabled={loading}>
        {loading ? <Spinner size="sm" /> : <><i className="bx bx-refresh me-1" />Refresh</>}
      </Button>
    </div>
  </div>
);
