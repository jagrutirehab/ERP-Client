...
const InternReceipt = ({ data }) => {
  const receipt = data?.receipt || {};
  const { totalAmount = 0, paymentModes = [] } = receipt;
        ...
        <Row>
          <Col xs={12}>
            <div className="d-flex justify-content-between align-items-center">
              ...
              <p className="font-size-25 text-primary">{totalAmount}</p>
            </div>
            <div className="text-center border shadow-lg p-1 mb-3">
              <h6 className="text-primary m-0 fs-xs-11 fs-md-14">
                Calculated Amount: {data?.calculatedAdvance ?? 0}
              </h6>
            </div>
