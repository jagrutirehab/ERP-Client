...
import { updateBiometricStatus } from "../../../helpers/backend_helper";

const ApproveBiometricModal = ({ isOpen, toggle, row, status, onSuccess }) => {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      await updateBiometricStatus(row?._id, {
        status,
    ...
    <Modal isOpen={isOpen} toggle={handleClose} centered>
      ...
      <ModalBody>
        <p className="text-muted mb-3">
          {isRejection
            ? "You are rejecting the biometric request for "
            : "You are approving the biometric request for "}
...
export default ApproveBiometricModal;
