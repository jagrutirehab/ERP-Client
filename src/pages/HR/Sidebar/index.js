...
const Sidebar = () => {
  ...
  const hasShiftRoasterPermission = hasPermission(
    "HR",
    "SHIFT_ROSTER",
    "READ"
  );
  ...
  const hasRaisedCompOffPerm = hasPermission(
    "HR",
    "RAISED_COMP_OFFS",
    "READ"
  );
  const hasCompOffHistoryPerm = hasPermission(
    "HR",
    "COMP_OFF_HISTORY",
    "READ"
  );
  ...
  const hasMyPayslipsPermission =
    hasPermission("HR", "EMPLOYEE_PAYSLIPS", "READ") 

  const hasEmployeePayslipsPermission =
    hasPermission("HR", "EMPLOYEE_PAYSLIPS", "READ") 
      ...
      if (page.id === "exit-employees") {
        page.children = page.children.filter((child) => {
          if (
            ...
            return false;
          if (child.id === "exit-approval" && !hasUserPermission4)
            return false;
      ...
      if (page.id === "leaves") {
        page.children = page.children.filter((child) => {
          ...
          if (child.id === "manage-leaves" && !hasManageLeavesPermission)
            return false;
          if (child.id === "my-leaves" && !hasMyLeavesPermission)
            return false;
          ...
          if (child.id === "comp-off-requests" && !hasCompOffRequestsPerm)
            return false;
          if (child.id === "my-comp-off" && !hasRaisedCompOffPerm)
            return false;
      ...
      if (page.id === "hr-dashboard") {
        page.children = page.children.filter((child) => {
          ...
          if (child.id === "comp-off-history" && !hasCompOffHistoryPerm)
            return false;
          if (
            child.id === "cancellations-history" &&
