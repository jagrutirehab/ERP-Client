...
const PendingApprovals = ({ activeTab }) => {
    ...
    const fetchPendingEmployeeTransferApprovals = async () => {
        try {
            ...
            await dispatch(fetchEmployeeTransfers({
                ...
                limit,
                centers,
                view: "TRANSFER_LOCATION_PENDING",
                ...search.trim() !== "" && { search: debouncedSearch }
            })).unwrap();
        } catch (error) {
            if (!handleAuthError(error)) {
                toast.error(error.message || "Failed to fetch outgoing employee records");
            }
        }
    };
    ...
    const handleAction = async () => {
        ...
        try {
            const response = await employeeTransferTransferLocationAction(selectedRecord._id, {
                action: actionType,
                note,
            });
    ...
    const columns = [
        ...
        ...(hasPermission("HR", "TRANSFER_EMPLOYEE_TRANSFER_LOCATION_APPROVAL", "WRITE")
            ? [
                {
                    ...
                    cell: row => (
                        <div className="d-flex gap-2">
                            <CheckPermission
                                ...
                                subAccess="TRANSFER_EMPLOYEE_TRANSFER_LOCATION_APPROVAL"
                                permission="edit"
                            >
                                <Button
                                    color="success"
