...
import ReturnMedicinesModal from "./ReturnMedicinesModal";
...
const ReturnableList = ({ activeTab, hasUserPermission }) => {
    ...
    const openReturnModal = (approvalId, centerId) => {
        ...
        setReturnModalCenterId(centerId);
    };
    ...
    const refetch = async () => {
        ...
        try {
            ...
            if (!handleAuthError(error)) {
                toast.error(error.message || "Failed to fetch approved medicines.");
            }
        }
    };
        ...
        canWrite("PHARMACY", "MEDICINE_RETURN") && {
            ...
            cell: (row) => (
                <Button
                    ...
                    onClick={() => openReturnModal(row._id, row.center?._id)}
                >
    ...
    return (
        <div className="px-3">
            ...
            <ReturnMedicinesModal
                isOpen={!!returnModalApprovalId}
                onClose={() => openReturnModal(null, null)}
                approvalId={returnModalApprovalId}
                centerId={returnModalCenterId}
                onDone={refetch}
            />
