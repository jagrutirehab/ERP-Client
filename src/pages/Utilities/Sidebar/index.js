...
import { UTILITIES } from "../../../Components/constants/pages";
...
const Utilitiessidebar = () => {
    ...
    const hasUploadBillPermission = hasPermission(
        "UTILITIES",
        "UPLOAD_BILL",
        "READ"
    );
    ...
    const filteredUtilitiesOptions = UTILITIES?.filter((page) => {
        if (page.id === "upload-bill") {
            if (!hasUploadBillPermission) return false;
            return true;
        }

        if (page.id === "reports") {
            if (!hasReportsPermission) return false;
            return true;
        }
    ...
    useEffect(() => {
        filteredUtilitiesOptions?.forEach((page) => {
            if (
                page?.isAccordion &&
            ...
            <div className="chat-leftsidebar">
                ...
                <PerfectScrollbar className="chat-room-list">
                    <ul className="list-unstyled chat-list chat-user-list users-list ps-4 pe-3 pt-2">
                        {filteredUtilitiesOptions.map((page) => {
