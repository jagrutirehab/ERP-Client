...
const getLeadBadge = (leadType = "") => {
  const normalized = leadType.trim().toLowerCase();
  if (normalized.includes("hot"))
    return { label: "Hot Lead", color: "#ee8e8e", bg: "#fef2f2" };
  if (normalized.includes("normal"))
    return { label: "Normal Lead", color: "#eec494", bg: "#fffbeb" };
  if (normalized.includes("cold"))
    return { label: "Cold Lead", color: "#9db8f3", bg: "#eff6ff" };
  return null;
};
    ...
    <div className="container-fluid px-4">
      <Card className="shadow-sm w-100">
        ...
        >
              ...
              onClick={async () => {
                if (prevRecording) {
                  ...
                  if (prevPageData.length > 0) {
                    navigate(
                      `/recordings/more/${prevPageData[prevPageData.length - 1]._id}`,
                      {
                        state: {
            ...
            !data?.Files?.geminiResponse?.startsWith("API Error") && (
              <Col md={12}>
                ...
                >
                  ...
                  >
                    {normalizeGeminiResponse(data?.Files?.geminiResponse)}
                  </div>
                </Section>
              </Col>
            )}
          {/* Lead Quality */}
          {(() => {
            const gemini = parseGeminiJson(data?.Files?.geminiResponse);
            const badge = getLeadBadge(gemini?.lead_type);
          ...
          <Row>
            ...
            <Col md={6}>
              <Section title="Agent Details">
                ...
                <Field
                  label="Agent Dial Status"
                  value={data?.Agent_Dial_Status}
                />
                <Field
                  label="Customer Dial Status"
                  value={data?.Customer_Dial_Status}
                />
