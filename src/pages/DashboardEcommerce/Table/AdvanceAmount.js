...
const MyAgGrid = () => {
  ...
  useEffect(() => {
    ...
    data.invoicedAmountAdvanceAmount.forEach(({ centerName, records }) => {
      ...
      records.forEach(({ month, year, advanceAmount }) => {
        if (month && year) {
          const key = `${month}-${year}`;
          monthYearSet.add(key);
          row[key] = advanceAmount;
        }
      });
  ...
  return (
    <div style={{ height: 600, width: '100%' }}>
       <h3 className="ag-theme-quartz" style={{ margin: '10px 0', textAlign: 'center' }}>Advance Amount Table</h3>
      <AgGridReact
        theme={myTheme}
