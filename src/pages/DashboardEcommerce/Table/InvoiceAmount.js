...
import { GET_ADVANCE_AMOUNT } from '../../../GraphQL/graphQl-Query/financeQueries';
...
const MyAgGrid = () => {
  const { data, loading, error } = useQuery(GET_ADVANCE_AMOUNT);
  const [columnDefs, setColumnDefs] = useState([]);
  const [rowData, setRowData] = useState([]);

  useEffect(() => {
    if (!data?.invoicedAmountAdvanceAmount?.length) return;
  
    const monthYearSet = new Set();
    const allRows = [];
  
    data.invoicedAmountAdvanceAmount.forEach(({ centerName, records }) => {
      const row = { centerName };
  
      records.forEach(({ month, year, invoiceAmount }) => {
        if (month && year) {
          const key = `${month}-${year}`;
          monthYearSet.add(key);
          row[key] = invoiceAmount;
        }
      });
  ...
  return (
    <div style={{ height: 600, width: '100%' }}>
       <h3 className="ag-theme-quartz" style={{ margin: '10px 0', textAlign: 'center' }}>Invoice Amount Table</h3>
      <AgGridReact
        theme={myTheme}
