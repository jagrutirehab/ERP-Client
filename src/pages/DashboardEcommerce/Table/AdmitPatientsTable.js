...
import { GET_TOTAL_PATIENT } from '../../../GraphQL/graphQl-Query/financeQueries';
...
const MyAgGrid = () => {
  const { data, loading, error } = useQuery(GET_TOTAL_PATIENT);
  const [columnDefs, setColumnDefs] = useState([]);
  const [rowData, setRowData] = useState([]);

  useEffect(() => {
    if (!data?.admitPatients?.length) return;
  
    const monthYearSet = new Set();
    const allRows = [];
  
    data.admitPatients.forEach(({ centerName, records }) => {
  ...
  return (
    <div style={{ height: 600, width: '100%' }}>
       <h3 className="ag-theme-quartz" style={{ margin: '10px 0', textAlign: 'center' }}>Admit Patient Table</h3>
      <AgGridReact
        theme={myTheme}
