...
export const MyLeavesColumn = (
  ...
  },
  {
    ...
    cell: (row) => (
      <Center>
        {row?.currentApprovalAuthority?.name ||
          row?.approvalAuthority?.name ||
          "-"}
      </Center>
    ),
    width: "180px",
  ...
  },
  {
    ...
    cell: (row) => {
      ...
      return (
        <Center>
          {row?.leave?.leaveType === "LEAVE_WTIHOUT_PAYS"
            ? "LEAVE_WITHOUT_PAYS"
            : row?.leave?.leaveType || "-"}
        </Center>
      );
  ...
  },
  {
    ...
    cell: (row) => (
      <Center>
        {row?.leave?.fromDate
          ? moment(row?.leave?.fromDate).format("DD-MM-YYYY")
          : "-"}
      </Center>
    ),
    width: "130px",
  },
  {
    ...
    cell: (row) => (
      <Center>
        {row?.leave?.toDate
          ? moment(row?.leave?.toDate).format("DD-MM-YYYY")
          : "-"}
      </Center>
    ),
    width: "130px",
    ...
    ? [
        {
          ...
          cell: (row) => (
            <Center>{row?.leave?.cancellationReason || "-"}</Center>
          ),
          width: "180px",
        },
      ]
