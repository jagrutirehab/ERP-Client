...
const Inflows = ({ centers, centerAccess, inflows, loading }) => {
    ...
    const hasCreatePermission =
        hasPermission("CASH", "CASHINFLOW", "WRITE") ||
        hasPermission("CASH", "CASHINFLOW", "DELETE");

    const hasReadPermission = hasPermission("CASH", "CASHINFLOW", "READ");
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, { resetForm }) => {
            ...
            try {
                await dispatch(addInflow({ formData, centers: centerAccess })).unwrap();
                resetForm();
                setAttachment(null);
                setAttachmentTouched(false);
                toast.success("Cash inflow logged successfully");
            } catch (error) {
                if (!handleAuthError(error)) {
                    toast.error(error.message || "Failed to log cash inflow.");
                }
            }
            resetForm();
            setAttachment(null);
    ...
    const data = inflows?.[cacheKey]?.data || [];

    return (
        <React.Fragment>
            <h5 className="fw-bold mb-3">Cash Inflows</h5>
            <Row>
                <CheckPermission
                    ...
                    <Col lg={4} className="mb-4">
                        <Card className="h-100 shadow-sm">
                            <CardHeader className="bg-transparent border-bottom">
                                <h5 className="mb-0 fw-semibold">Log New Cash Inflow</h5>
                            </CardHeader>
                            <CardBody>
                                ...
                                >
                                    {loading ? (
                                        <div className="text-center py-5 text-muted">
                                            ...
                                            <p className="h5">Fetching Cash inflows...</p>
                                        </div>
                                    ) : data?.length === 0 ? (
                                        <div className="text-center py-5 text-muted">
                                            <Receipt size={48} className="mb-3" />
                                            <p className="h5">No cash inflow recorded yet</p>
                                            <p>Start by logging your first cash inflow using the form.</p>
                                        </div>
                                    ) : (
                                        data?.map((spending) => (
                                            <ItemCard
...
const mapStateToProps = (state) => ({
    ...
    inflows: state.Cash.inflows,
});

export default connect(mapStateToProps)(Inflows);