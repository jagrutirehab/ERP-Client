...
import { fetchCityVisitedDate } from "../../../store/features/miReporting/miReportingSlice";
...
const CityVisitedDate = () => {
  const dispatch = useDispatch();
  const { cityVisitedDate, loading } = useSelector(
    (state) => state.MIReporting
  );
  const [date, setDate] = useState(moment().format("YYYY-MM"));

  useEffect(() => {
    dispatch(fetchCityVisitedDate({ date }));
  }, [dispatch, date]);
  ...
  return (
    <div className="page-content">
      <Container fluid>
        <BreadCrumb title="City Visited Date" pageTitle="MI Reporting" />
        <Row>
          <Col lg={12}>
            <Card>
              <CardBody>
                ...
                {loading ? (
                  ...
                  <div className="table-responsive">
                    <Table className="table-bordered table-nowrap table-hover">
                      ...
                      <tbody>
                        {cityVisitedDate.length > 0 ? (
                          cityVisitedDate.map((item, index) => (
                            <tr key={index}>
                              <td>{item.center}</td>
                              {uniqueDates.map((d) => {
...
export default CityVisitedDate;
