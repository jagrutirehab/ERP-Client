...
const SubDepartmentList = ({ onAdd, onEdit }) => {
  ...
  useEffect(() => {
    ...
    const fetchSubDepartments = async () => {
      setLoading(true);
      try {
        const res = await getPRSubDepartments({ search });
        if (cancelled) return;
        setSubDepartments(res?.data || []);
      } catch (error) {
        if (cancelled) return;
        if (!handleAuthError(error)) {
          toast.error(error?.response?.data?.message || error?.message || "Couldn't load sub-departments.");
        }
      } finally {
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true);
    try {
      await deletePRSubDepartment(deleteTarget._id);
      toast.success("Sub-department deleted successfully");
      setDeleteTarget(null);
      setRefreshFlag((f) => f + 1);
    } catch (error) {
      if (!handleAuthError(error)) {
        toast.error(error?.response?.data?.message || error?.message || "Couldn't delete sub-department.");
      }
    } finally {
      setDeleting(false);
    }
  ...
  const columns = [
    ...
    {
      name: "Department",
      cell: (row) => <span className="uom-cell-muted">{row.departmentId?.name || "—"}</span>,
    },
    {
      name: "Actions",
      width: "120px",
    ...
    <div>
      <div className="d-flex justify-content-between align-items-center mb-3">
        <div className="uom-search-wrap mb-0">
          ...
          <Input placeholder="Search sub-departments..." value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
        {canCreate && (
          <Button color="primary" onClick={onAdd}>
            <i className="bx bx-plus me-1"></i> Add Sub-Department
          </Button>
        )}
      </div>
        ...
        <DataTable
          ...
          noDataComponent={
            <div className="uom-empty-state">
              <p className="uom-empty-title">No sub-departments found</p>
              <p className="uom-empty-sub">Try adjusting your search, or add your first sub-department.</p>
            </div>
          }
        />
      </div>
      ...
      <Modal isOpen={!!deleteTarget} toggle={() => setDeleteTarget(null)} centered>
        <ModalBody className="p-4">
          <h5 className="mb-2">Delete this sub-department?</h5>
